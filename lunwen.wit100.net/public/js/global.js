...
{
    var upurl ='/Admin/Uploadify/poster_upload?num='+num+'&input='+elementid+'&path='+path+'&func='+callback+'&fileType='+fileType;
    var title = '上传图片';
    if(fileType == 'Flash'){
        title = '上传视频';
    }
...
function GetUploadify2(num,elementid,path,callback,url)
{	   	
...
function get_category(id,next,select_id){
    ...
    if(id == 0){
        var html = "<option value='0'>请选择机械分类</option>";
        $('#'+next).empty().html(html);
        return false;
    }
    $.ajax({
        type : "GET",
        url  : '/Home/api/get_category?parent_id='+ id,
        dataType:'json',
