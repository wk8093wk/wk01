...

{
...

    {
        ...
        $user = Db::name('users')->where("mobile", $username)->whereOr('user_name', $username)->find();
        ...
        if (!$user) {
            ...
            $levelName = Db::name("user_level")->where("level_id", $levelId)->Field("level_name")->find();
			...
			//exit();
			 ...
			 Db::name('users')->where("user_id", $user['user_id'])->data($newArr)->update();
...
//            $comment_count   = M('order_goods')->where("order_id", $add['order_id'])->where('is_comment', 0)->count();
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true){
		...
		$row=M('users')->where(array('user_id'=>$user_id))->update(array('password'=>encrypt($new_password)));
