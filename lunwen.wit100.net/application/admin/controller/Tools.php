...
class Tools extends Common
{
    ...
    public function index_json()
    {
        ...
        $res=array('code'=>0,'msg'=>'','count'=>count($list),'data'=>$list,'total'=> format_bytes($total)); 
        $this->ajaxReturn($res);
    }
    ...
    public function export($tables = null, $id = null, $start = null)
    {
        ...
        if(IS_POST && !empty($tables) && is_array($tables)){ //初始化
            ...
            if(is_file($lock)){
                return json(array('msg'=>'检测到有一个备份任务正在执行，请稍后再试！', 'code'=>1, 'url'=>''));
            } else {
                //创建锁文件
            ...
            if(!is_writeable($config['path'])){
                return json(array('msg'=>'备份目录不存在或不可写，请检查后重试！', 'code'=>1, 'url'=>''));
            }
            ...
            if(false !== $Database->create()){
				return json(array('msg'=>'参数有误', 'code'=>1, 'url'=>''));
                $speed = (floor((1/count($tables))*10000)/10000*100).'%';
                $tab = array('id' => 0, 'start' => 0, 'speed'=>$speed, 'table'=>$tables[0]);
                return json(array('tables' => $tables, 'tab' => $tab, 'msg'=>'初始化成功！', 'code'=>0, 'url'=>''));
            } else {
                return json(array('msg'=>'初始化失败，备份文件创建失败！', 'code'=>1, 'url'=>''));
            }
            ...
            if(false === $start){ //出错
                ...
                if(isset($tables[++$id])){
                    ...
                    return json(array('tab' => $tab, 'msg'=>'备份完成！', 'code'=>0, 'url'=>''));
                } else { //备份完成，清空缓存
    ...
    public function repair()
    {
        ...
        if (!DB::query("REPAIR TABLE {$strTable} ")) {
            $res=array('code'=>1,'msg'=>'修复失败'); 
           $this->ajaxReturn($res);
        }else{
			$res=array('code'=>0,'msg'=>'修复成功'); 
            $this->ajaxReturn($res);
		}
...
//                    array_map("unlink", glob($path));
    ...
    {
        $time_arr = input('time');
        $time_arr = strtotime($time_arr);
        if(!empty($time_arr)){
           
                $name  = date('Ymd-His',$time_arr) . '-*.sql*';
                $path = tpCache('global.web_sqldatapath');
                $path = !empty($path) ? $path : config('DATA_BACKUP_PATH');
                $path = trim($path, '/');
                $path  = realpath($path) . DS . $name;
