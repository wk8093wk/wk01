...
class project2 extends Common
{
    ...
    { 
	  ...
	  $sort_by=I('post.sort_by','project2_id');
	  ...
	  $count = M('project2')->where($where)->count();
	  $page  = new Page($count,$page_size);
	  $logs = M('project2')->where($where)->limit($page->firstRow,$page->listRows)->order($sort_by.' '.$sort_order)->select(); 
	  foreach($logs as $k=>$v){
	...
	public function art_del(){
		$id = I('post.id',0);
		$row=M('project2')->update(array('project2_id'=>$id,'is_delete'=>1));
		if($row){
		...
		$ids = I('post.ids');
		//$ids=implode(",", $id);
		$row=M('project2')->where('project2_id','in',$ids)->update(array('is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
	...
	public function index_listorder(){
		...
		$info['project2_id'] = I('get.id');
		$row=M('project2')->data($info)->update();
		if($row){
	...
	public function edit(){
		if (Request::post()){
			...
			if($param){
			 ...
			 $data['project2_id']=$param['project2_id'];
			 
			 M('project2')->update($data);
	    ...
	    $id = I('get.id/d',0);
	    $info = M('project2')->where('project2_id',$id)->find();
	...
	public function add(){
		if (Request::post()){
			...
			$data['content']=$param['content'];
			 ...
			 $raw=M('project2')->data($data)->insert();
	...
	public function relevant(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info = M('project2')->where('project2_id',$id)->field('name')->find();
    ...
    { 
	 $catid = I('get.catid');
	 $this->assign('catid',$catid); 
	 $info = M('project2')->where('project2_id',$catid)->field('name')->find();
	 $this->assign('catname',$info['name']);
     return $this->fetch();
    }
	...
	public function vote(){
		$id = I('get.catid',0);
		$this->assign('id',$id);
		$info = M('project2_annex')->where('id',$id)->field('name')->find();
		$this->assign('catname',$info['name']);
		return $this->fetch();
	}
	...
	public function guanzhu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info_name = M('project2')->where('project2_id',$id)->field('name')->find();
	...
	public function tousu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info2 = M('project2')->where('project2_id',$id)->field('name')->find();
	...
	public function project2_annex(){
		$id = I('get.catid',0);
		$this->assign('id',$id);
		$info = M('project2')->where('project2_id',$id)->field('name')->find();
		$this->assign('catname',$info['name']);
	...
	public function project2_annex_json(){
	  $page = I('post.page',1);
	  $page_size=I('post.limit',10);
	  $key_word=I('post.key','');
	  $sort_by=I('post.sort_by','id');
	  ...
	  $count = M('project2_annex')->where($where)->count();
	  $page  = new Page($count,$page_size);
	  $logs = M('project2_annex')->where($where)->limit($page->firstRow,$page->listRows)->order($sort_by.' '.$sort_order)->select(); 
	  foreach($logs as $k=>$v){
		 $logs[$k]['add_time']=date('Y-m-d H:i:s',$v['add_time']);
	...
	public function project2_annex_edit(){
		if (Request::post()){
			$param = I('post.');
			if($param){
			 $data['name']=$param['name'];
			 $data['annex']=$param['annex'];
			
			 $data['id']=$param['id'];
			 M('project2_annex')->data($data)->update();
			 $res=array('code'=>0,'msg'=>'编辑成功'); 
             $this->ajaxReturn($res); 
	    ...
	    $id = I('get.id/d',0);
	    $info = M('project2_annex')->where('id',$id)->find();
		
		$this->assign('info',$info);
        return $this->fetch();
	}
	
	public function project2_annex_add(){
		if (Request::post()){
			$param = I('post.');
			if($param){
			 $data['name']=$param['name'];
			 ...
			 M('project2_annex')->data($data)->insert();
			 $res=array('code'=>0,'msg'=>'添加成功'); 
             $this->ajaxReturn($res); 
	...
	public function project2_annex_dels(){
		$id = I('post.id',0);
		$ids=implode(",", $id);
		$row=M('project2_annex')->where('id','in',$ids)->update(array('is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
	...
	public function project2_annex_del(){
		$id = I('post.id',0);
		$row=M('project2_annex')->update(array('id'=>$id,'is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
	...
	public function project2_annex_listorder(){
		$info = I('get.',0);
		
		$row=M('project2_annex')->data($info)->update();
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'更新成功！']);
		}else{
			$this->ajaxReturn(['list'=>$info]);
