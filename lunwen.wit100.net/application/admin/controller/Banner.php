...
class Banner extends Common
{
    ...
    { 
	  ...
	  $sort_by=I('post.sort_by','banner_id');
	  ...
	  $logs = M('banner')->where($where)->limit($page->firstRow,$page->listRows)->order($sort_by.' '.$sort_order)->select(); 
	  foreach($logs as $k=>$v){
	...
	public function art_del(){
		$id = I('post.id',0);
		$row=M('banner')->update(array('banner_id'=>$id,'is_delete'=>1));
		if($row){
	...
	public function art_dels(){
		$ids = I('post.ids');
		//$ids=implode(",", $id);
		$row=M('banner')->where('banner_id','in',$ids)->update(array('is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
	...
	public function edit(){
	    ...
	    $id = I('get.id/d',0);
	    $info = M('banner')->where('banner_id',$id)->find();
	...
	public function add(){
		...
		$banner_id = I('get.id',0);
		$hide = I('get.hide',0);
		
		$row=M('banner')->data(array('banner_id'=>$banner_id,'hide'=>$hide))->update();
		if($row){
	...
	public function relevant(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info = M('banner')->where('banner_id',$id)->field('name')->find();
	...
	public function relevant_edit(){
	    ...
	    $id = I('get.id/d',0);
	    $info = M('news')->where('banner_id',$id)->find();
	...
	public function relevant_del(){
		$id = I('post.id',0);
		$row=M('news')->update(array('banner_id'=>$id,'is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
    ...
    { 
	 $catid = I('get.catid');
	 $this->assign('catid',$catid); 
	 $info = M('banner')->where('banner_id',$catid)->field('name')->find();
	 $this->assign('catname',$info['name']);
     return $this->fetch();
    }
	...
	public function guanzhu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info_name = M('banner')->where('banner_id',$id)->field('name')->find();
	...
	public function tousu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info2 = M('banner')->where('banner_id',$id)->field('name')->find();
