...
class Database extends Common
{
    ...
    public function optimize() {
        ...
        if (empty($tables)) {
            return ['code'=>1,'msg'=>'请选择要优化的表！'];
        }
        if($this->db->optimize($tables)){
            return ['code'=>0,'msg'=>'数据表优化成功！'];
        }else{
            return ['code'=>1,'msg'=>'数据表优化出错请重试！'];
        }
    }
    //修复
    public function repair() {
        $tables = input('tables/a');
        if (empty($tables)) {
            return ['code'=>1,'msg'=>'请选择要修复的表！'];
        }
        if($this->db->repair($tables)){
            return ['code'=>0,'msg'=>'数据表修复成功！'];
        }else{
            return ['code'=>1,'msg'=>'数据表修复出错请重试！'];
        }
    }
    //备份
    ...
    public function delSqlFiles() {
        ...
        if($this->db->delFile($time)){
            return ['code'=>0,'msg'=>"备份文件删除成功！"];
        }else{
            return ['code'=>1,'msg'=>"备份文件删除失败，请检查权限！"];
        }
    }
}