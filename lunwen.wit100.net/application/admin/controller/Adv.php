...
class Adv extends Common
{
    ...
    { 
	  ...
	  $logs = M('adv')->where($where)->limit($page->firstRow,$page->listRows)->order($sort_by.' '.$sort_order)->select(); 
	  foreach($logs as $k=>$v){
		 $logs[$k]['start_time']=date('Y-m-d H:i:s',$v['start_time']);
		 $logs[$k]['end_time']=date('Y-m-d H:i:s',$v['end_time']);
	...
	public function art_del(){
		$id = I('post.id',0);
		$row=M('adv')->update(array('adv_id'=>$id,'is_delete'=>1));
		if($row){
	...
	public function art_dels(){
		$ids = I('post.ids');
		//$ids=implode(",", $id);
		$row=M('adv')->where('adv_id','in',$ids)->update(array('is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
	...
	public function edit(){
	    ...
	    $id = I('get.id/d',0);
	    $info = M('adv')->where('adv_id',$id)->find();
	...
	public function relevant(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info = M('adv')->where('adv_id',$id)->field('name')->find();
	...
	public function relevant_edit(){
	    ...
	    $id = I('get.id/d',0);
	    $info = M('news')->where('adv_id',$id)->find();
	...
	public function relevant_del(){
		$id = I('post.id',0);
		$row=M('news')->update(array('adv_id'=>$id,'is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
    ...
    { 
	 $catid = I('get.catid');
	 $this->assign('catid',$catid); 
	 $info = M('adv')->where('adv_id',$catid)->field('name')->find();
	 $this->assign('catname',$info['name']);
     return $this->fetch();
    }
	...
	public function guanzhu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info_name = M('adv')->where('adv_id',$id)->field('name')->find();
	...
	public function tousu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info2 = M('adv')->where('adv_id',$id)->field('name')->find();
