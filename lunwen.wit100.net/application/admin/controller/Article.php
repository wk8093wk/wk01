...
class Article extends Common
{
    ...
    { 
	  ...
	  $sort_by=I('post.sort_by','article_id');
	  ...
	  $logs = M('article')->where($where)->limit($page->firstRow,$page->listRows)->order($sort_by.' '.$sort_order)->select(); 
	  foreach($logs as $k=>$v){
	...
	public function art_del(){
		$id = I('post.id',0);
		$row=M('article')->update(array('article_id'=>$id,'is_delete'=>1));
		if($row){
	...
	public function edit(){
		if (Request::post()){
			...
			if($param){
			 ...
			 $data['index_top_img']=$param['index_top_img'];
			 $data['total_enrollment']=$param['total_enrollment'];
			 $data['total_number']=$param['total_number'];
			 $data['music']=$param['music'];
			 $data['tc_pic']=$param['tc_pic'];
			 $data['tc_ewm']=$param['tc_ewm'];
			 $data['tanchuang']=$param['tanchuang'];
	...
	public function add(){
		if (Request::post()){
			...
			if($param){
			 ...
			 $raw=M('article')->data($data)->insert();
			 
			 if($raw){
				 
				 $catId = M('article')->getLastInsID();
			     $now=time();
	...
	public function relevant(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info = M('article')->where('article_id',$id)->field('name')->find();
	...
	public function relevant_edit(){
	    ...
	    $id = I('get.id/d',0);
	    $info = M('news')->where('article_id',$id)->find();
	...
	public function relevant_del(){
		$id = I('post.id',0);
		$row=M('news')->update(array('article_id'=>$id,'is_delete'=>1));
		if($row){
          $this->ajaxReturn(['code'=>0,'msg'=>'删除成功！']);
		}
	}
    ...
    { 
	 $catid = I('get.catid');
	 $this->assign('catid',$catid); 
	 $info = M('article')->where('article_id',$catid)->field('name')->find();
	 $this->assign('catname',$info['name']);
     return $this->fetch();
    }
	...
	public function guanzhu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info_name = M('article')->where('article_id',$id)->field('name')->find();
	...
	public function tousu(){
		$id = I('get.id',0);
		$this->assign('id',$id);
		$info2 = M('article')->where('article_id',$id)->field('name')->find();
